...
def execute():
    try:
        ...
        daily_threshold_value = frappe.db.get_single_value(
            "System Settings", "custom_flrts_daily_cost_threshold"
        )
        monthly_threshold_value = frappe.db.get_single_value(
            "System Settings", "custom_flrts_monthly_cost_threshold"
        )

        # Validate daily threshold
        try:
            daily_threshold = (
                float(daily_threshold_value) if daily_threshold_value is not None else 10.00
            )
            # Clamp to valid range [0.0, +inf)
            if daily_threshold < 0.0:
                frappe.logger().warning(
                    f"FLRTS Daily Cost Monitor: Invalid daily threshold {daily_threshold}, clamping to 0.0"
                )
                daily_threshold = 0.0
        except (ValueError, TypeError) as e:
            frappe.logger().warning(
                f"FLRTS Daily Cost Monitor: Invalid daily threshold value '{daily_threshold_value}', using default 10.00: {str(e)}"
            )
        ...
        try:
            monthly_threshold = (
                float(monthly_threshold_value) if monthly_threshold_value is not None else 300.00
            )
            # Clamp to valid range [0.0, +inf)
            if monthly_threshold < 0.0:
                frappe.logger().warning(
                    f"FLRTS Daily Cost Monitor: Invalid monthly threshold {monthly_threshold}, clamping to 0.0"
                )
                monthly_threshold = 0.0
        except (ValueError, TypeError) as e:
            frappe.logger().warning(
                f"FLRTS Daily Cost Monitor: Invalid monthly threshold value '{monthly_threshold_value}', using default 300.00: {str(e)}"
            )
            monthly_threshold = 300.00

        # Get recipients with fallback chain
        cost_alert_emails = frappe.db.get_single_value(
            "System Settings", "custom_flrts_cost_alert_emails"
        )
        if cost_alert_emails:
            alert_emails = [
                email.strip() for email in cost_alert_emails.split(",") if email.strip()
            ]
        else:
            # Fallback to custom_flrts_alert_emails
            general_alert_emails = frappe.db.get_single_value(
                "System Settings", "custom_flrts_alert_emails"
            )
            if general_alert_emails:
                alert_emails = [
                    email.strip() for email in general_alert_emails.split(",") if email.strip()
                ]
            else:
        ...
        if not alert_emails:
            frappe.logger().warning(
                "FLRTS Daily Cost Monitor: No recipients found, skipping alert email"
            )
            return
        ...
        if alert_needed:
            ...
            # Send email
            frappe.sendmail(recipients=alert_emails, subject=subject, message=message)

            frappe.logger().info(
                f"Cost alert sent: ${total_cost:.2f} today, projected ${projected_monthly:.2f} monthly"
            )
        else:
            frappe.logger().info(
                f"Cost monitoring: ${total_cost:.2f} today, projected ${projected_monthly:.2f} monthly - no alert needed"
            )
