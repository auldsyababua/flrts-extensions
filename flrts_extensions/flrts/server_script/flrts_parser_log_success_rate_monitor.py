...
def execute():
    ...
    try:
        ...
        threshold_value = frappe.db.get_single_value('System Settings', 'custom_flrts_success_rate_threshold')
        try:
            threshold = float(threshold_value) if threshold_value is not None else 80.0
            # Clamp to valid range [0.0, 100.0]
            if threshold < 0.0:
                frappe.logger().warning(f"FLRTS Success Rate Monitor: Invalid threshold {threshold}, clamping to 0.0")
                threshold = 0.0
            elif threshold > 100.0:
                frappe.logger().warning(f"FLRTS Success Rate Monitor: Invalid threshold {threshold}, clamping to 100.0")
                threshold = 100.0
        except (ValueError, TypeError) as e:
            frappe.logger().warning(f"FLRTS Success Rate Monitor: Invalid threshold value '{threshold_value}', using default 80.0: {str(e)}")
            threshold = 80.0
        
        # Get logs from last 24 hours
        yesterday = add_days(now(), -1)
        logs = frappe.db.get_all('FLRTS Parser Log',
        ...
        if accepted + rejected == 0:
            frappe.logger().info("FLRTS Success Rate Monitor: No completed parses in last 24 hours, skipping alert")
            return
...
"""
            ...
            custom_emails = frappe.db.get_single_value('System Settings', 'custom_flrts_alert_emails')
            if custom_emails:
            ...
            if not recipients:
                frappe.logger().warning("FLRTS Success Rate Monitor: No recipients found, skipping alert email")
                return
        ...
        frappe.logger().info(f"FLRTS Success Rate Monitor: success_rate={success_rate:.1f}%, threshold={threshold}%, alert_sent={alert_sent}")
        ...
        try:
            ...
            custom_emails = frappe.db.get_single_value('System Settings', 'custom_flrts_alert_emails')
            if custom_emails:
                error_recipients = [email.strip() for email in custom_emails.split(',') if email.strip()]
            else:
                system_manager_ids = frappe.get_all('Has Role',
                    filters={'role': 'System Manager'},
                    pluck='parent'
